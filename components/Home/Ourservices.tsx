...
const Ourservices = () => {
    return (
        <div className='px-20 bg-gray-100 py-14'>
            <p className="text-base text-primary font-bold text-center">Our best services</p>
            <h2 className="text-5xl font-[antonio] text-center mb-8">We Have Best Service for Succeed Quality</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 px-4 py-8">
                {data.map((item) => (
                    <div key={item.id} className="flex flex-col  border-primary items-center p-4 border bg-white shadow-md rounded-sm">
                        <div className="text-5xl text-primary mb-4">
                            {item.icon}
                        </div>
                        <h3 className="text-2xl text-primary font-bold mb-2">{item.title}</h3>
                        <p className="text-gray-500 text-base text-center">{item.description}</p>
                    </div>
                ))}
            </div>
        </div>
